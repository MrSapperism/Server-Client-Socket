...
for conf in $configDir/*.xml; do
	echo "$conf"
	echo "running the reference implementation (./ds-sim/ds-client)..."
	sleep 1
	if [ -z $2 ]; then
		./ds-sim/ds-server -c $conf -v brief > $conf.ref.log&
		sleep 1
		java Client
	else
		./ds-sim/ds-server -c $conf -v brief -n > $conf.ref.log&
		sleep 1
		java Client -n
	fi
	...
	sleep 2
	if [ -z $2 ]; then
		./ds-sim/ds-server -c $conf -v brief > $conf.your.log&
	else
		./ds-sim/ds-server -c $conf -v brief -n > $conf.your.log&
	fi
	sleep 1
	if [ -f $1 ] && [[ $1 == *".class" ]]; then
		java $(sed 's/\.class//' <<< $1)
